...
function Typewriter({ strings, speed = 80 }) {
  ...
  return <span>{display}<span className="caret">▮</span></span>;
}
    ...
    <>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Project card left-border reveal */
        ...
        .hero-bottom { display: flex; flex-wrap: wrap; gap: 20px; align-items: center; margin-top: 40px; }
        .hero-divider { height: 1px; width: 60px; background: var(--border); flex-shrink: 0; }
        ...
        @media (max-width: 900px) {
          ...
          .skill-row    { grid-template-columns: 56px 1fr 44px 80px; gap: 14px; }
        }
        ...
        @media (max-width: 640px) {
          ...
          /* Skills — hide bar column, keep tag + name + % */
          .skill-row        { grid-template-columns: 52px 1fr 44px; gap: 12px; }
      ...
      <section id="home" style={{ position: "relative" }}>
        <div className="hero">
          <motion.div className="hero-status" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            ...
            <span className="mono muted" style={{ fontSize: 11, letterSpacing: "0.1em" }}><span className="green">●</span> AVAILABLE FOR WORK</span>
          </motion.div>
          ...
          <div className="stats-grid">
            {[
              ...
              <div key={i} className={`stat-cell${i % 2 === 0 ? " stat-b-r" : ""}${i < 2 ? " stat-b-b" : ""}`}>
                ...
                <p className="mono muted" style={{ fontSize: 10, marginTop: 8, letterSpacing: "0.12em" }}>{stat.label.toUpperCase()}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <motion.section id="skills" className="section"
        ...
        {skillData.map((skill, i) => (
          <motion.div key={i} className="skill-row"
            ...
            <span className="mono green" style={{ fontSize: 11, textAlign: "right" }}>{skill.level}%</span>
      ...
      <motion.section id="projects" className="section"
        ...
        <h2 className="syne" style={{ fontSize: "clamp(28px, 5vw, 52px)", fontWeight: 800, marginBottom: 56 }}>SELECTED WORK</h2>

        {projects.map((proj, i) => (
          <motion.div key={i} className="proj-card proj-row"
            ...
            <span className="syne proj-num" style={{ fontSize: 32, fontWeight: 800, color: "var(--border)", lineHeight: 1 }}>{proj.id}</span>
          ...
          <div>
            <div style={{ marginBottom: 28 }}>
              ...
              <a href="mailto:abijithbinu654@gmail.com"
                className="syne link-grow" style={{ fontSize: "clamp(14px, 2.5vw, 20px)", fontWeight: 700, color: "var(--white)", textDecoration: "none" }}>
